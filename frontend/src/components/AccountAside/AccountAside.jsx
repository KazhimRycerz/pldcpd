...
return (
  <div>
    ...
    {isAuth && (knowledgeData ? (
      <aside id="homeAsideAccount" className={asidePos}>
        <img src={baseURL + userData.userImage} alt={userData.userName} />
        <p>
          <strong>Hallo, {contactData.firstName}</strong>
          <br />
          {knowledgeData.profession} <br />
          Karrierelevel <br />
          --- <span> {knowledgeData.myCStatus} </span> ---<br />
          {knowledgeData.careerPathStatus}
        </p>
        <br />
        ...
        <div id="home_data">
          ...
          <div>
            <div>
              myKF <p id="myKF">{knowledgeData.myKF}</p>
            </div>
            <div>
              myLF <p id="myLF">{knowledgeData.myLF}</p>
            </div>
            <div>
              myPEDh <p id="myPED">{knowledgeData.myPEDh}</p>
            </div>
            <div>
              myPEXh <p id="myPEX">{knowledgeData.myPEXh}</p>
            </div>
          </div>
          <div>
            <div>
              <p id="maKF">{marketData.maKF}</p>maKF
            </div>
            <div>
              <p id="maLF">{marketData.maLF}</p>maLF
            </div>
            <div>
              <p id="maPED">{marketData.maPEDh}</p>maPED
            </div>
            <div>
              <p id="maPEX">{marketData.maPEXh}</p>maPEX
            </div>
          </div>
        ...
        <p>
          Die Werte vergleichen Ihren Stand des Wissens mit dem Durchschnitt aller teilnehmenden Lichtdesigner. Derzeit liegt Ihr WissensFaktor KF{" "}
          {knowledgeData.myKF > marketData.maKF ? (
            <span>über dem Marktdurchschnitt. </span>
          ) : (
            <span>unter dem Marktdurchschnitt.</span>
          )}
        </p>

        <p>
          Ihr aktuelles <span className="LitCoin">L</span>
          <span className="Calli">it</span>
          <span className="LitCoin">C</span>
          <span className="Calli">oin</span> Guthaben:
        </p>
        <div id="aside_gutbox">
        ...
        <p>
          Ihre gesamte Übersicht Ihres Kontos können Sie{" "}
          <Link to="/KnowledgeAccount">hier</Link> aufrufen.{" "}
          <Link className="C" to="/KnowledgeAccount">
            C
          </Link>
        </p>
      </aside>
      ...
      <aside id="homeAsideAccount" className={asidePos}>
        ...
        <div id="home_data">
          ...
          <div>
            ...
            <div>
              myPEDh <p id="myPED">0</p>
            </div>
            <div>
              myPEXh <p id="myPEX">0</p>
            </div>
          </div>
          <div>
            <div>
        ...
        <p>
          Ihre Wissensdaten wurden noch nicht angelegt. Zum Aktivieren bitte{" "}
          <Link to="/KnowledgeAccount">hier</Link> klicken!{" "}
        </p>
      </aside>
    ))}
  </div>
